...
`AQUI <https://angus.readthedocs.io/en/2019/R_Intro_Lesson.html#vectors>`_

Usaremos R y RStudio en nuestras instancias en la nube. Hemos preinstalado RStudio en la imagen del taller. Inicie un Jetstream m1.medium o más grande usando la imagen ANGUS 2019 según las instrucciones de inicio de Jetstream. Conéctese a su instancia.

Necesitaremos nuestro nombre de usuario de instancia para conectarnos a RStudio. Para determinar cuál es su nombre de usuario, ejecute:

.. code-block:: bash

    echo My username is $USER

El enlace para la interfaz del servidor RStudio se construye con la dirección IP de nuestra instancia (por defecto en el puerto 8787), la cual podemos obtener desde la terminal ejecutando el siguiente comando:
...
``hostname`` es un comando en la terminal que obtendrá información variada sobre nuestras computadoras. Al usarlo con la opción ``-i``, se obtiene la **dirección IP** de nuestra computadora. Por lo tanto, si lo encierra entre paréntesis y lo precede el símbolo ``$``, significa que la línea de comandos ejecutará primero ese comando, colocará la salida de ese comando en su lugar (nuestra dirección IP) y luego ejecutará el comando echo, que luego imprimirá el enlace apropiado a nuestro servidor RStudio.
...
Usaremos R y RStudio en la nube. Para usar RStudio en su computadora, puede descargar R y RStudio. Si no tiene R y RStudio instalados:

-   Descargar R desde
    `CRAN <https://cran.r-project.org/>`_
-   Ir a la página de `descarga de RStudio <https://posit.co/download/rstudio-desktop/#download>`_ e instala la versión que sea compatible con tu sistema operativo:
    Este `enlace <https://datacarpentry.github.io/R-ecology-lesson/>`_ proporciona más instrucciones sobre la instalación y configuración de R y RStudio en su computadora.


Introduction to R and RStudio
-----------------------------

R es un lenguaje de programación y un entorno de software libre para gráficos y computación estadística. Muchos programas de bioinformática se implementan en R. RStudio es un entorno de desarrollo integrado (IDE) gráfico que muestra y organiza más información sobre R. R es el lenguaje de programación subyacente. Una vez que inicie RStudio, verá este diseño (será similar tanto en su instancia como en su computadora):


<center><img src="_static/RStudio_first.png" width="90%"></center>
<br>

Los scripts son como un libro de laboratorio digital. Registran todo lo que ejecutas, así como notas para ti mismo sobre cómo y por qué lo ejecutaste. Para iniciar un nuevo script, puedes hacer clic en el ícono que parece una página con un "+" debajo de Archivo y seleccionar -> Script R. Esto genera un archivo .R. Los archivos .R son archivos de texto sin formato que usan la extensión de archivo .R para que los humanos puedan recordar que el archivo contiene código R.
...
Aunque los scripts son un registro de nuestro código, también incluyen comentarios sobre él. Estos comentarios son muy importantes, ya que le recuerdan a usted (y a quienquiera que le pase su script) por qué hizo lo que hizo. Piense en los comentarios como las notas que toma en su cuaderno de laboratorio cuando hace experimentos.

<center><img src="_static/RStudio_type_script.png" width="90%"></center>
<br>

Cuando ejecutas una línea de tu script, esta se envía a la consola. Si bien el script es un registro permanente de todo lo que ejecutaste, la consola es el motor que ejecuta esas cosas. Puedes ejecutar código desde un script o desde la consola, pero solo se registra en scripts.

Hay dos paneles adicionales en RStudio:

1.  **Panel Entorno/Historial/Conexiones**

    -   **Panel de entorno**: Este panel registra los objetos que están cargados actualmente en nuestro entorno RStudio. Esto incluye cualquier objeto de datos que hayamos creado y la información básica sobre ellos.


    -   **Panel de historial**: Esto tiene un resultado similar al comando history que usamos en el shell. Registra todos los comandos que hemos ejecutado en la consola R.


2.  **Files/Plots/Packages/Help/Viewer** El panel nos proporciona un espacio para interactuar con los gráficos y archivos que creamos y con los archivos de ayuda integrados en R.


    -   **Packages Pane**: Un lugar para organizar los paquetes dentro de RStudio. Permite una fácil instalación o carga (library()) de los paquetes que tenemos.


¡Y ahora estamos listos para comenzar la parte divertida!
...
Un objeto R es algo que creas y al que le asignas un valor o una función. Para ello, utilizas el operador de asignación, que es <- (NOTA: atajo Alt + -). Por ejemplo, puedes asignar un valor numérico a un objeto escribiendo en tu script (recuerda comentar lo que estás haciendo):
...
También puedes asignar un valor *carácter* a un objeto:
...
Para ejecutar una línea de código, coloque el cursor en la línea que desea ejecutar y presione Enter+Cmd o Enter+Ctrl. O seleccione la línea y haga clic en el botón que dice Run (Ejecutar) en la esquina superior derecha de su script.
...
Ahora revisa las pestañas Consola y Entorno, ¡verás que los comandos se han ejecutado y que has creado nuevos objetos!


Puedes hacer todo tipo de cosas con tus objetos: operaciones, combinaciones, usarlos en funciones. A continuación te mostramos cómo realizar una multiplicación usando un objeto. Como a almacena el valor 34, podemos multiplicarlo por 2:
...
Las funciones son “scripts enlatados” que automatizan conjuntos de comandos más complicados, incluidas las asignaciones de operaciones, etc. Muchas funciones están predefinidas o se pueden poner a disposición importando paquetes R (más sobre esto más adelante). Una función generalmente toma una o más entradas llamadas argumentos. Las funciones a menudo (pero no siempre) devuelven un valor. Un ejemplo típico sería la función sqrt(). La entrada (el argumento) debe ser un número y el valor de retorno (de hecho, la salida) es la raíz cuadrada de ese número. Ejecutar una función ('ejecutarla') se llama llamar a la función. Un ejemplo de una llamada de función es:
...
Aquí, el valor de a se le asigna a la función sqrt(), que calcula la raíz cuadrada y devuelve el valor que luego se asigna al objeto b. Esta función es muy simple, ya que solo acepta un argumento.


El valor de retorno de una función no tiene por qué ser numérico (como el de sqrt()) y tampoco tiene por qué ser un único elemento: puede ser un conjunto de elementos o incluso un conjunto de datos. Veremos esto cuando leamos archivos de datos en R.


De manera similar al shell, los argumentos pueden ser cualquier cosa, no solo números o nombres de archivos, sino también otros objetos. El significado exacto de cada argumento difiere según la función y debe buscarse en la documentación (ver más abajo). Algunas funciones toman argumentos que pueden ser especificados por el usuario o, si se omiten, toman un valor predeterminado: estos se denominan opciones. Las opciones se utilizan normalmente para modificar la forma en que opera la función, como por ejemplo si ignora los "valores incorrectos" o qué símbolo utilizar en un gráfico. Sin embargo, si desea algo específico, puede especificar un valor de su elección que se utilizará en lugar del valor predeterminado.
...
Los argumentos son las "condiciones" de una función. Por ejemplo, la función round() tiene un argumento para determinar el número de decimales, y su valor predeterminado en R es 0. Aquí, hemos llamado a round() con un solo argumento, 3.14159, y ha devuelto el valor 3. Esto se debe a que el valor predeterminado es redondear al número entero más cercano. Si queremos más dígitos, podemos ver cómo hacerlo obteniendo información sobre la función round. Podemos usar args(round) para encontrar qué argumentos toma, o mirar la ayuda para esta función usando ?round.
...
Itâ€™s best practice to put the non-optional arguments (the data that you
...
logical â†’ numeric â†’ character â† logical
...
("â€œ), or asâ€NA" before reading them into R.
...
This tutorial is a continuation from our *Salmon* lesson â€“ we are using
...
Although weâ€™re only working with 6 samples, this was a much larger
...
authors made their data avaialble and â€˜linkableâ€™ between technical and
...
Letâ€™s investigate your new data using some exploratory functions:
...
-   `%>%`: pipes, they allow â€˜concatenatingâ€™ functions  
...
We have now a table with many columns â€“ but what if we donâ€™t need all of
...
So letâ€™s store that in an object named `experiment_info`:
...
```
    ...
    ## # A tibble: 6 x 7
    ##   Sample `Yeast Strain` `Nucleic Acid Coâ€¦  A260  A280 `260/280` `Total RNA`
...
â€œpipeâ€ in R, which is represented by `%>%`. **Note:** This is the R
...
this. Letâ€™s convert our units into micrograms/microliter:
...
```
    ...
    ##   <chr>           <chr>          <dbl> <chr>           <chr>           
    ## 1 PRJEB5348       ERR458493       4932 Saccharomyces â€¦ Illumina HiSeq â€¦
    ## 2 PRJEB5348       ERR458494       4932 Saccharomyces â€¦ Illumina HiSeq â€¦
    ## 3 PRJEB5348       ERR458495       4932 Saccharomyces â€¦ Illumina HiSeq â€¦
    ## 4 PRJEB5348       ERR458496       4932 Saccharomyces â€¦ Illumina HiSeq â€¦
    ## 5 PRJEB5348       ERR458497       4932 Saccharomyces â€¦ Illumina HiSeq â€¦
    ## 6 PRJEB5348       ERR458498       4932 Saccharomyces â€¦ Illumina HiSeq â€¦
    ## # â€¦ with 9 more variables: library_layout <chr>, read_count <dbl>,
...
```
    ...
    ##   study_accession run_accession tax_id scientific_name instrument_model
    ##   <chr>           <chr>          <dbl> <chr>           <chr>           
    ## 1 PRJEB5348       ERR459201       4932 Saccharomyces â€¦ Illumina HiSeq â€¦
    ## 2 PRJEB5348       ERR459202       4932 Saccharomyces â€¦ Illumina HiSeq â€¦
    ## 3 PRJEB5348       ERR459203       4932 Saccharomyces â€¦ Illumina HiSeq â€¦
    ## 4 PRJEB5348       ERR459204       4932 Saccharomyces â€¦ Illumina HiSeq â€¦
    ## 5 PRJEB5348       ERR459205       4932 Saccharomyces â€¦ Illumina HiSeq â€¦
    ## 6 PRJEB5348       ERR459206       4932 Saccharomyces â€¦ Illumina HiSeq â€¦
    ## # â€¦ with 9 more variables: library_layout <chr>, read_count <dbl>,
    ## #   experiment_alias <chr>, fastq_bytes <dbl>, fastq_md5 <chr>,
    ## #   fastq_ftp <chr>, submitted_ftp <chr>, sample_alias <chr>,
    ## #   sample_title <chr>
...
```
    ...
    ## 5 PRJEB5348       ERR458497       4932 Saccharomyces â€¦ Illumina HiSeq â€¦
    ## 6 PRJEB5348       ERR458498       4932 Saccharomyces â€¦ Illumina HiSeq â€¦
    ## # â€¦ with 12 more variables: library_layout <chr>, read_count <dbl>,
    ## #   experiment_alias <chr>, fastq_bytes <dbl>, fastq_md5 <chr>,
    ## #   fastq_ftp <chr>, submitted_ftp <chr>, sample_alias <chr>,
...
```
    ...
    ## 1 ERR458493     ena-EXPERIMENT-â€¦    1093957    59532325 2b8c708câ€¦     1
    ## 2 ERR458500     ena-EXPERIMENT-â€¦    1885330   102201086 107aad97â€¦     1
    ## 3 ERR458507     ena-EXPERIMENT-â€¦    1594160    86382961 93bca10aâ€¦     1
    ## 4 ERR458514     ena-EXPERIMENT-â€¦    1594695    86462515 48fa134bâ€¦     1
    ## 5 ERR458521     ena-EXPERIMENT-â€¦    1977253   106992945 98a9d336â€¦     1
    ## 6 ERR458528     ena-EXPERIMENT-â€¦    1371825    74704461 e4bf39d6â€¦     1
    ## # â€¦ with 2 more variables: yeast_strain <chr>, biol_rep <dbl>
...
â€˜ERR458493â€™,â€˜ERR458494â€™,â€˜ERR458495â€™,â€˜ERR458500â€™,â€˜ERR458501â€™and
â€™ERR458502â€™.
...
the variables to be plotted and how they will be plotted (e.g.Â x/y,
size, shape, colorâ€¦)...
